...
export default function Dashboard({ setItemsForStats }) {
    ...
    async function loadAll() {
      ...
      try {
        ...
        const revealed = await Promise.all(
          vault.map(async (it) => {
            try {
              const { password } = await api(`/vault/${it.id}/reveal`);
              return { ...it, revealed: password };
            } catch { return it; }
          })
        );
        setItems(revealed);
        setItemsForStats?.(revealed);
      } catch { setItems([]); }
  ...
  const { total, strong, medium, weak, reused, risky } = useMemo(() => {
    ...
    const revealed = items.filter(it => it.revealed);
    revealed.forEach(it => {
      const lvl = computeStrength(it.revealed);
      if (lvl === 'STRONG') strong++;
      else if (lvl === 'MEDIUM') medium++;
      else weak++;
    });
    const pwdCount = {};
    revealed.forEach(it => { pwdCount[it.revealed] = (pwdCount[it.revealed] || 0) + 1; });
    const reusedPwds = new Set(Object.keys(pwdCount).filter(p => pwdCount[p] > 1));
    const reused = revealed.filter(it => reusedPwds.has(it.revealed));
    const riskyIds = new Set([
      ...revealed.filter(it => computeStrength(it.revealed) === 'WEAK').map(it => it.id),
      ...reused.map(it => it.id),
    ]);
    const risky = items.filter(it => riskyIds.has(it.id));
    return { total: items.length, strong, medium, weak, reused, risky };
  }, [items]);

  const score = total === 0 ? null : Math.round((strong / total) * 100);
  const scoreColor = score === null ? 'var(--muted)' : score >= 70 ? 'var(--success)' : score >= 40 ? 'var(--gold)' : 'var(--danger)';
                ...
                <div className={`item ${lvl === 'WEAK' ? 'weak' : 'medium'}`} key={it.id}>
                  ...
                  <div style={{ display: 'flex', gap: '.4rem', alignItems: 'center' }}>
                    ...
                    <button
                      ...
                      onClick={() => navigate('/add', { state: { id: it.id, website: it.website, username: it.username } })}
                    >
                      Fix →
                    </button>
                  </div>
